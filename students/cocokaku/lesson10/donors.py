"""
updated Donors class for Lesson 10 assignment (functional programming)
(1) added classmethod challenge: multiplies every donation in database by a factor,
    may optionally use min_donation and/or max_donation to filter donations to multiply,
    saves to a new database
(2) added method total_value: calculates total dollar value of database 
...
class Donors(dict):
    ...
    @classmethod
    def challenge(cls, db, factor, min_donation=None, max_donation=None):
        new_db = Donors()
        for d in db.values():
