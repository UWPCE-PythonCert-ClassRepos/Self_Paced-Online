...
class Donors():
    ...
    def donations(self):
        ...
        for donor in self._donor_list:
            donor_donations = donor.donation
...
def run_func():
    while True:
        ...
        if response == 1:
            ...
            dl = d1.donations()
            abv_match = reduce((lambda x, y: x + y), map(lambda x: x * 2, filter(lambda x: x >=100, dl)))
            blw_match = reduce((lambda x, y: x + y), map(lambda x: x * 3, filter(lambda x: x <=50, dl)))
            print(10 * "-" + " Match Report " + 10 * "-")
            print("Doubling all donations $100 or above would be: ${}".format(abv_match))
            print("Tripling all donations $50 or above would be: ${}".format(blw_match))
        elif response == 7:
            break
