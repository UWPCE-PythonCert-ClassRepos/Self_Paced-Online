...
class Donors:
    ...
    def get_donor_summary(donors):
        ...
        for d in donors:
            ...
            str_total = Helpers.format_currency_str(total)
            str_number = str(len(donations))
            str_average = Helpers.format_currency_str(total / max(number, 1))
            summary.append([name, str_total, str_number, str_average])
        return summary

    @staticmethod
    def donor_email(name, donations):
        donations = Helpers.format_currency_str(donations)
        m = ('\n\nDear {},\n\n'
    ...
    def generate_letters(self):
        ...
        for key, val in self.donors.items():
            ...
            donations = Helpers.format_currency_str(total_donations)
    ...
    def donations_total(self, name=None):
        return Helpers.format_currency_str(sum(self.donors[name]))
    ...
    def donations_average(self, name=None):
        ...
        return Helpers.format_currency_str(total / count)
...
class Donor(Donors):
    ...
    def donors_update(self, name, donation):
        """ Update base donor dictionary """
        if self.donor_key_found(name) is False:
            # new donor
            self.donors[name] = [float(donation)]
        else:
            # existing donor
            self.donors[name].append(float(donation))

    @staticmethod
    def donor_email(name, donation):
        donation = Helpers.format_currency_str(donation)
        m = ('\n\nDear {},\n\n'
...
class MailroomCli:
    ...
    def process_new_donor(self):
        ...
        while True:
            ...
            if D.donor_key_found(name) is False:
                break
            else:
    ...
    def thank_donor(self, name=None, amount=None):
        ...
        if name is not None:
            ...
            while True:
                name = self.prompt_name()
                if D.donor_key_found(name) is True:
                    amount = D.donors[name]
                    break
                else:
                    print('Type the exact donor name (see list): ', end='\n\n')
                    D.list_donors()
            D.donor_email(name, sum(amount))  # total donations
