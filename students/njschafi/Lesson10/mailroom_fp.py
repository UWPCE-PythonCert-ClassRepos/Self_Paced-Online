#!/usr/bin/env python3
# NEIMA SCHAFI, LESSON 10 Assignment Mailroom
import sys
...
class Donor():
    ...
    def f_donations(self, min_value, max_value):
        """Filters Donations"""
        if min_value:
            self.initlist = list(filter(
            lambda x: x >= min_value, self.donationlist))
        if max_value:
            self.initlist = list(filter(
            lambda x: x <= min_value, self.donationlist))

    def multiply(self, factor):
        """Multiples donations by factor"""
        self.__donationlist = list(map(
        lambda x: x * factor, self.__donationlist))
...
class Donors():
    ...
    def challenge(self, factor, min_value=False, max_value=False):
        """Multiplies donor's donations by a given factor,
        returns new donor object with updated values"""
        n = deepcopy(Donors())
        for donor in n.donors:
            donor.f_donations(min_value, max_value)
            donor.multiply(factor)
        return n

    def all_donations(self):
        """Sums all donations given by donors and returns that value"""
        self.total = 0
        for donor in self.donors:
            self.total += donor.amount
        return round(self.total, 2)

    def projection(self):
        """Ask user for inputs in order to projection matching values."""
        print('Please enter the min and max contributions to match'
                ' and the matching factor.')
        min_value = float(input('Minimum contribution to match:\n'))
        max_value = float(input('Maximum contribution to match:\n'))
        factor = float(input('Matching Factor:\n'))
        match = self.challenge(factor, min_value, max_value)
        print('\nSum of all donations given ${}.'.format(self.all_donations()))
        print('Based on the inputted parameters you will match: '
                '${}'.format(match.all_donations()))
