...
def test_render_element():
    ...
    assert file_contents.count("</html>") == 1  # needs to be "2" if line 32
#       remains aligned with except; 1 if aligned with For statement in render Class
    print(file_contents)
#    assert False    
...
def test_oneliner():
    e = OneLineTag("Center Content")
#    with pytest.raises(NotImplementedError): Diagnostic option entry
#        e.append("-error check-")
    e.append("-end-")
    file_contents = render_result(e).strip()
    print(file_contents)    
    assert("Center Content") in file_contents
#    assert("-end-") in file_contents
    """
    You should not be able to append content to a OneLineTag
    """
    assert file_contents.startswith("<oneliner>")
    assert file_contents.endswith("</oneliner>")    


def test_title():
    e = Title("this is TITLE")
    e.append("and more TITLE")

    file_contents = render_result(e).strip()

    assert("this is TITLE") in file_contents
    assert("and more TITLE") in file_contents

    assert file_contents.startswith("<title>")
    assert file_contents.endswith("</title>")

########
# Step 4  Attributes- Keywords
########
...
def test_H():   #Broken   'Not sure why
