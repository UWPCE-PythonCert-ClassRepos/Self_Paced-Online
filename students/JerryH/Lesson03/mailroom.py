    ...
    while op > 3 or op < 1:
        ...
        if op == THANK_YOU_OPT: #Append or list donors
            ...
            while d_name.lower() == "list": #provide a list of donors if "list" is entered
                ...
                for each_name in sorted(get_all_donor_names(donors)):
            ...
            if d_name not in get_all_donor_names(donors):
                #print(donors)
                donors.append([d_name, [donation]])
            else:
                #print(get_all_donor_names(donors).index(d_name))
                donors[get_all_donor_names(donors).index(d_name)][1].append(donation)
