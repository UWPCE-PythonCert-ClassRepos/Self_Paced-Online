...
def test_head_element():
    """
    test that text is in correct format
    """
...
def test_one_line_tag_append():
    """
    You should not be able to append to text
    """
    e = OneLineTag("some content")
    with pytest.raises(NotImplementedError):
        e.append("new content")

    file_contents = render_result(e).strip()
    print(file_contents)

#Step 4 Attribute Tests
def test_attributes():
    ...
    assert file_contents.index(">") > file_contents.index('id="intro"')
...
def test_append_content_in_br():
    with pytest.raises(TypeError):
        br = Br()
        br.append("other content")

#Step 6: Create an A class for an anchor (link) element. 
#Its constructor should look like:
#A(self, link, content)
...
#Step 7
#Header elements
"""You should not be able to append text, but since this is a subclass of 
OneLineTag, and I've tested for that on that class, I don't need to test again

I'm going to test if the class instance works"""
