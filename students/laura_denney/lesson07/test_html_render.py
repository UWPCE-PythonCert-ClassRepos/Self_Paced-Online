...
def test_attributes():
    p = P("here is some text", style="text-align: center; font-style: oblique;")

    file_contents = render_result(p).strip()

    assert ("here is some text") in file_contents
...
def test_selfclosingtag_normal():
    e = Hr(style="text-align: center; font-style: oblique;")
...
def test_ul():
    u = Ul("This is the second item", style="color: red")

    file_contents = render_result(u).strip()
    assert "This is the second item" in file_contents
    assert file_contents.startswith('<ul style="color: red">')
