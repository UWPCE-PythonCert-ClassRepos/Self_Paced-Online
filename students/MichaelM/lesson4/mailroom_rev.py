...
def spam_donors():
    """
        adds a year end thankyou letter for each donor to a subdirectory
    ...
    for donor in list_donors():
        sum_amt = 0.0
        for donor2 in esteemed_donors:
            if donor2[0] == donor:
                sum_amt = sum_amt + donor2[1]
        tmp_dest_file = f"{tmp_directory}/{donor}_Thank_You_Letter.txt"
        with open(tmp_dest_file, 'w') as wf:
            wf.write(spam("all_donors", donor, 0.0, "", sum_amt))
