...
def create_report():
    ...
    print(header)
    print("-" * (len(header) - 1))
    # pack the donors into a list of tuples for soting
    donors_list = [(name, donors[name]) for name in donors]
    donors_list.sort(key=total_donations, reverse=True)
    [print("{:<20}| ${:>10.2f} | {:>9} | ${:>11.2f}".format(donor[0], donor[1][0], len(donor[1]) - 1, donor[1][0]/(len(donor[1])-1))) for donor in donors_list]
