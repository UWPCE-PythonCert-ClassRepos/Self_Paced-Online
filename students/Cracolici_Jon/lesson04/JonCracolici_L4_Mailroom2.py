...
def start_user_input():
    ...
    return task
#Functions relating to the Send a Thank You Menu
def saty_control(dB):
...
def new_donor(newdonordict, dB):
    ...
    dB.update(updict)
    #Important note - if run using an input dict containing info for an existing
    #donor this function will overwrite that donor's previous info. I intend to add error handling
    #for this situation in lesson 5.
    return dB
...
def show_current_names(dB):
    ...
    #names = [ k['donor'] for k in dB.keys()]
    names = []
