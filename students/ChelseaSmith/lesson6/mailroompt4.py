...
def report(dictin):  # this prints a report given database dictin
    ...
    for k in dictin:
        tot = sum(dictin[k])  # switched to using built in functions to clean up code. did not find a spot to put in comprehensions
        num = len(dictin[k])
        print(f"{k:<18}${tot:12.2f}{num:10}   ${(tot / num):13.2f}")
...
def thankyou(dictin):  # collects all actions for sending a thank you
    ...
    try:
        ...
        print("Error! Non-numerical value entered for donation amount. Letter cannot be created.")
    else:
