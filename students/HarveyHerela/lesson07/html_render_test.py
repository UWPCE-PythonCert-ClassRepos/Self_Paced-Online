...
class HtmlRenderTest(unittest.TestCase):
    ...
    def test_hlevels(self):
        # Valid range is H1 through H6
    ...
    def test_self_closing(self):
        ...
        for t in tags:
            ...
            page.render(rendered)
            self.assertEqual(rendered.getvalue(), expected)
            self.assertRaises(TypeError, page.append, "lorem ipsum")
    ...
    def test_content(self):
        # Everything is based on Element, so if it works
        # on Element, then it'll work on everything
        expected = '<element>\nHello \n\n</element>'
