...
def test_render_element():
    ...
    # making sure that the program is rendering correctly:
...
#test OneLineTag's overriding append method:
...
#Step 4
#test if the attributes are rendered correctly
...
def test_html():
    ...
    # making sure the opening line says <!DOCTYPE html>.
    assert file_contents.startswith("<!DOCTYPE html>")
    # making sure that the closing tag is correct
    assert file_contents.endswith("</html>")
...
def test_meta_attr():
    """a horizontal rule with an attribute"""
    char = "UTF-8"
    meta = Meta(charset=char)
    file_contents = render_result(meta)
    print(file_contents)
    assert file_contents == '<meta charset="{}" />\n'.format(char)
