...
class Donor:
    ...
    @property
    def donation_record(self):
        """Return a list containing all of the donations this donor has made."""
        return list(self._donor_record)
...
""" 
    ...
    def challange(self, factor, min_donation=-1.0*math.inf, max_donation=math.inf):
        """Return a new DonorDatabase identical to the current database with the
        exception that all donation amounts are scaled by factor.
        
        Inputs:
        min_donation - Only include donations that are greater than min_donation
        max_donation - Only include donations that are less than max_donation
        """
        
        new_db = DonorDatabase()
        
        stdout_orig = sys.stdout
        with open(os.devnull,'w') as output:
            sys.stdout = output
            for donor in self._donor_dict:
                
                # Filter out (remove) all of the donations for the current donor that are
                # either less than min_donation or greater than max_donation
                filtered_donations = filter(lambda d_i: d_i > min_donation, \
                                            self._donor_dict[donor].donation_record)
                filtered_donations = filter(lambda d_i: d_i < max_donation, \
                                            filtered_donations)
                for scaled_donation in map(lambda d_i: d_i*factor, filtered_donations):
                    new_db.add_donation(donor, scaled_donation)
    ...
    def database_sum(self):
        """Return the sum of the donations for all of the donors in the database."""
        
        d_sum = 0
        for donor in self._donor_dict:
            d_sum += self._donor_dict[donor].sum_donations
...
class Menu:
    ...
    def get_numerical_input(prompt):
        """Display the specified prompt to request the user for a numerical value.
        
        If the user specifies to quit, then a value of None is returned, otherwise
        the numerical value requested is returned."""
        
        in_value = input(prompt)
        if in_value == 'q': return None
...
>>> """
    ...
    def __init__(self, session):
        ...
        self._dispatch_dict = dict(self._dispatch_dict,
                                   ...
                                   p=MainMenu._projected_contribution)
    ...
    @staticmethod
    def get_projection_bounds(prompt):
        """Prompt the user for a lower or upper donation limit for the projection.
        
        If the user specifies to quit, then a value of None is returned, otherwise
        the numerical value requested is returned. If no bound is specified, infinity 
        is returned."""
        
        in_value = input(prompt)
        if in_value.lstrip() == '': return math.inf
        if in_value == 'q': return None
    ...
    def _projected_contribution(self):
        """Ret"""
        
        factor_prompt = "\nEnter an amount to factor donations by (or 'q' to quit)" \
                        "\n>>>"
        factor_in = Menu.get_numerical_input(factor_prompt)
        if factor_in is None: return None
        
        lower_limit_prompt = "\nInclude only donations greater than this amount" \
                  "\n(or <return> for no limit, 'q' to quit):" \
                  "\n>>> "
        lower_limit_in = MainMenu.get_projection_bounds(lower_limit_prompt)
        # The lower limit should be negative infinity if no value is specified
        if lower_limit_in is None: 
            return None
        elif lower_limit_in == math.inf:
            lower_limit_in = -1.0*lower_limit_in
            
        
        upper_limit_prompt = "\nInclude only donations less than this amount" \
                  "\n(or <return> for no limit, 'q' to quit):" \
                  "\n>>> "
        upper_limit_in = MainMenu.get_projection_bounds(upper_limit_prompt)
        if upper_limit_in is None: return None
        
        # Get a copy of the database for the projected values
        projection_db = self._donor_db.challange(factor_in, lower_limit_in, upper_limit_in)
        # Print the sum of all of the donations in the projected database
        print('\nProjected value of funds matched: ${:.2f}'.format(projection_db.database_sum))
...
>>> """
    ...
    def _create_entry(self):
        ...
        donation_prompt = "\nEnter the donation amount (or 'q' to quit)" \
                  ...
                  "\n>>> "
        amount_in = Menu.get_numerical_input(donation_prompt)
        if amount_in is None: return None
