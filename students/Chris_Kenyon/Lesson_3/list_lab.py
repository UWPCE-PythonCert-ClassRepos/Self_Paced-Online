...
add_fruit = input('Enter an additional fruit name: ')
...
while True:
    try:
        user_num = int(input('Pick a fruit in the list between 1 and {}: '
                       .format(len(series_1))))
    except ValueError:
...
add_fruit_2 = input('Enter a fruit to add to the front of the list: ')
...
while True:
    rem_fruit = input('Please select a fruit from the list to remove: ')
...
for f in series_3:
    while True:
        yorn = input('Do you like {}? (y/n): '.format(f.lower()))
        if yorn[0].lower() == 'n':
            gross_fruit += [f]
