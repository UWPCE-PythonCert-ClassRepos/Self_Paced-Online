# ----------------------------------------------------------------------------------------------------------------------
# NAME: MICAH BRAUN
# PROJECT: mailroom_refactor.py - Lesson 06 Assignment
# PURPOSE: TDD - Unit testing
# DATE: 07/10/2018
# EDITS MADE:
#
# 07/11/2018 -- write
#
# 07/10/2018 -- refactored thankyou() broken up in to 3 functions (add_donors, add_donoation, thankyou)
# this was done to make unit testing more manageable and to make the code easier to read.
#
# 06/29/2018 -- Try, Except blocks added for exception handling, .get() removed from menu(),
# 'pair' var updated to handle error on quitting before assigning donation value to new name entry in thankyou()
#
#  DESCRIPTION: Mailroom.py takes user input and expands list of donor information to be displayed/manipulated
#  back to the user.
# ----------------------------------------------------------------------------------------------------------------------
...
def menu():
    ...
    optD = "[D] Quit"
    whole_menu_header = header + '\n' + title + '\n' + header  # header strung together as one unit
    ...
    while True:
        ...
        switch_function = {
            'A': add_donors,  # dictionary holding menu options
            'B': createreport,
            'C': writeletters,
            'D': quit_program,
        }
        ...
        if fname_prompt.upper() == "Q":  # if Q, then quit to menu()
            ...
            if lname_prompt.upper() == "Q":  # if Q, back to menu
                ...
                if key in donor_dict.keys():
                    existing_donor = input("That value is already in the list! Do you want to "
                                           "proceed with that selection? (Y/N): ")
...
def add_donation(name):
    ...
    donation_amt_str = input("Enter in the donation amount from Donor {0}: $".format(name))
    ...
    try:
        ...
        print('{0} has donated ${1:,.2f}'.format(name, donation_amt))  # display name and donation amt
        int(donation_amt)
        thankyou(name, donation_amt)


def thankyou(donor, donation):
    ...
    for item in range(0, len(donor_dict)):  # set index var to current name
        if list(donor_dict.values())[item][0] == donor:
            get_index = item
            break
    firstname = list(donor_dict.values())[get_index][0].split(' ', ).pop(0)  # separate first name
    current_donations = int(list(donor_dict.values())[get_index][1])  # current amt/ var from dict{[val]}
    sum_donations = current_donations + donation  # sum of all donations (current + new)
...
def get_directory():
    """Obtain or assign default directory for writeletters()"""
    default = os.getcwd()  # get and assign current directory to variable
    default = default + "\\"  # format default (will work with Windows)
    try:
...
def writeletters():
    ...
    while True:
        ...
        if msg.upper() == "Y":  # check input
            ...
            for x in range(int(len(donor_dict))):  # for items in span donor_dict
                firstname = list(donor_dict.values())[x][0]  # firstname = donor_dict at index[item], column[0]
                firstname = firstname.split(' ', ).pop(0)  # format to split on whitespace
                first_names.append(firstname)  # append value to empty list
            ...
            for names in range(1, int(len(names_list))):  # omit header, for each name in names_list[]
                try:
                    with open(cur_dir + str(names_list[names]) + '.txt',
                    ...
                    print('Error on line {}'.format(sys.exc_info()[-1].tb_lineno), type(e).__name__, e)
            print("Files being written to " + cur_dir + "...")
            print("Done!")

        elif msg.upper() == "N":  # if "N" for msg = input()
            input("Exiting back to menu [Press ENTER]")  # leaving this function
            break  # back to menu()
        else:
            print("That's an unrecognized value! Only Y or N are accepted.")
            continue  # warning message: back to beginning of function
...
'''
#   NOTE: I could not get these 2 functions to work together... I tried breaking up the displaylist() function
          into a processing function and a display function to better separate things for test-ability but
          I kept running into typeErrors, inaccessible values within if-blocks, and the final attempt yielded
          just blank returns. Not sure what to do with this section
...
def return_list():
    ...
    if int(len(list(donor_dict.values()))) - 1 % 2 != 0:  # even/odd check
        for i in range(0, int(len(list(donor_dict.values())) - int(len(list(donor_dict.values())) - 2) / 2)):
            if i == 0:  # header check
                list_header = spacing + '{:>44s}'.format(str(list(donor_dict.values())[i][0])) + spacing
                combine.append(list_header)
            elif cut_off + i >= len(list(donor_dict.values())):  # if cut_off + i exceeds len(list)
                uneven_left_col_handler = '{:>30s}'.format(list(donor_dict.values())[i][0])  # i (not cut_off + i)
                combine.append(uneven_left_col_handler)
                continue
            else:
                uneven_right_col_handler = '{:>30s}'.format(list(donor_dict.values())[i][0]), \
                                           '{:>35s}'.format(list(donor_dict.values())[cut_off + i][0])
                combine.append(uneven_right_col_handler)
        return combine
        ...
        for i in range(0, int(len(list(donor_dict.values())) / 2)):
            if i == 0:  # header check
                ...
                col_handler = '{:>30s}'.format(list(donor_dict.values())[i][0]), \
                                           '{:>35s}'.format(list(donor_dict.values())[cut_off + i][0])
                combine.append(col_handler)
    return combine


def print_list():
    """Function returns current dictionary contents (names only)"""
    list_display = return_list()
    print(list_display)
'''
