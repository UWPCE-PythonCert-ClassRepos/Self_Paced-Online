...
def menu():
    ...
    while True:
        ...
        print(optA + optB.center(32, ' ') + optC.center(30, ' ') + optD.center(21, ' ') + '\n')

        usr_sel = str(input("Menu Selection: ")).upper()        # 'select case' menu
        ...
        if usr_sel == "A":
            ...
            # usr_sel != "A" or usr_sel != "B" or usr_sel != "C":
                ...
                for i in range(0, int(len(data_list) - 1 / 2)):  # list len(odd), i in size of list - 1/ 2
                    ...
                    if i == 0:  # check for header
                        ...
                        print('{:>30s}'.format(data_list[i][0]),
                              '{:>35s}'.format(data_list[cut_off + i][0]))  # print out names
            else:
            ...
            if lname_prompt.upper() == "Q":  # if Q, back to menu
                ...
                if len(data_list) - 1 % 2 != 0:  # check length of list, if odd print list -
                    for i in range(0, int(len(data_list) - 1 / 2)):  # -odd justified
                        ...
                        if i == 0:  # header check
                            ...
                            print('{:>30s}'.format(data_list[i][0]),
                                  '{:>35s}'.format(data_list[cut_off + i][0]))  # display names
                ...
                if full_name in search_names:  # search list of only names for full_name
                    ...
                    if existing_donor.upper() == "Y":  # if user proceeds, print display
                        ...
                        average = (data_list[get_index][1] * data_list[get_index][2]) / data_list[get_index][
                            2]  # calculate avg
...
Foundation Board of Directors\n'.format(data_list[get_index][0], data_list[get_index][1]))
                    ...
                    if name_found is False:
                        add_name = input(
                            "That name is not in the Donor list. Do you want to add it to the list?  ").upper()
                        if add_name == "Y":  # if input response == "Y", proceed
                            ...
                            if len(data_list) - 1 % 2 != 0:  # even/odd check
                                for i in range(0, int(
                                        len(data_list) - (len(data_list) - 2) / 2)):  # to be honest, I do not
                                    ...
                                    if i == 0:  # header check
                                        ...
                                        print('{:>30s}'.format(data_list[i][0]),
                                              '{:>35s}'.format(data_list[cut_off + i][0]))
                            else:
                                if i == 0:  # list length even
                                    ...
                                    print('{:>15s}'.format(data_list[i][0]),
                                          '{:>30s}'.format(data_list[cut_off + i][0]))

                            add_donation_count = 1  # add new item to new name index
                            donation_amt = int(
                                input("Enter in the donation amount from Donor {0}: $".format(full_name)))
                            ...
                            donation_print = '{:.2f}'.format(data_list[get_index][1])
                            firstname = (
                                data_list[get_index][0].split(' ', ).pop(0))  # separate first name, create variable
                            # lastname = (data_list[get_index][0].split(' ', ).pop(1))             # last name, create variable not used
                            ...
                            if email_display.upper() == "Y":  # print email message
                                ...
                                print('Dear {0}, \n\nThank you for your continued support through your \
contribution of ${1} towards our Foundation\'s fundraising goal.\n\nBest wishes,\n\
Foundation Board of Directors\n'.format(firstname, donation_print))
                                print(spacing)
...
def writeletters():
    names_list = []                         # empty list to hold list of data_list names w/o whitespace (for filenames)
    ...
    while True:
        ...
        if msg.upper() == "Y":              # check input
            ...
            for x in range(int(len(data_list))):                    # for items in the span of data_list
                firstname = data_list[x][0]                         # firstname = data_list at index[item], column[0]
            ...
            for i in range(int(len(data_list))):                    # for items in the span of data_list
                elem = data_list[i][0]                              # elem = data_list at index[i], column[0]
            ...
            for names in range(1, int(len(names_list))):            # omit header (range 0), for each name in names_list[]
                with open(working_directory + str(names_list[names]) + '.txt', 'a') as filename:    # create files/file-names
                    ...
                    filename.write('\n\nDear {0}, \n\nThank you for your continued support through your contribution '
                                   ...
                                   'Foundation Board of Directors'
                                   '\n'.format(str(first_names[names]), locale.currency(data_list[names][1], grouping=True)))
