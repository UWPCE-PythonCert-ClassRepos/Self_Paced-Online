...
def remove_first_last_every_other_middle(seq):
    """
    Return a copy of the sequence with the first 4 and the last 4 items
    removed, and then every other item in between.
...
def reverse_sequence(seq):
    """
    Return a copy of the sequence with the elements reversed (just with
    slicing).
...
def mid_last_first(seq):
    """
    Return a copy of the sequence with the middle third, then last third, then
    ...
    # Use the third plus multiples of a third along with the total size to
    # rearrange the sequence. Using the total length accomadates lengths that
    # aren't evenly divisible by 3.
...
tuple_2 = ("one", "two", "three", "four", 1, 2, 3, 4, 5, 6, "seven", "eight",
           "nine", "ten")
