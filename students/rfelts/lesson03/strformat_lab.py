...
# Step 1 & 2
# Use string formatting to convert the tuple data to:
# 'file_002 :   123.46, 1.00e+04, 1.23e+04'
#
# The first element is used to generate a filename. Pad the numbers with zeros
# to get the right sort order.
#
# The second element is a float displayed with 2 decimal places.
#
# The third value is an integer displayed with in scientific notation, with 2
# decimal places shown.
#
# The fourth value is a float displayed in scientific notation with 3
# significant figures.
...
fstring_tuple = (f"file_{tuple_1[0]:03d}, {tuple_1[1]:.2f}," +
                 f"{tuple_1[2]:.2e}, {tuple_1[3]:.2e}")
print('Step 2:\nFormatting the tuple:', tuple_1,
...
def format_tuple(numbers_tuple):
    """
    Dynamically build up the format string to accommodate formatting any tuple
    length.

    Parameters:
    arg1 (tuple) The tuple to format

    Returns:
    string: A dynmically formatted string
    """
...
# Step 4
# Given a 5 element tuple: ( 4, 30, 2017, 2, 27) use string formating to print:
# '02 27 2017 04 30'

tuple_4 = (4, 30, 2017, 2, 27)
print(("Step 4: " + f"\'{tuple_4[3]:02d} {tuple_4[4]:02d} " +
       f"{tuple_4[2]:d} {tuple_4[0]:02d} " +
       f"{tuple_4[1]:d}\'\n"))
...
# Then change the f-string so that it displays the names of the fruit in upper
...
name_padding = len(max(name_list, key=len))
name_padding += 4
