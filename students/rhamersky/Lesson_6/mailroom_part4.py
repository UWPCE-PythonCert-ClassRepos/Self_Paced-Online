    ...
    def menu_options():
        ...
        while True:
            ...
            # Verify that the user selected an option 1 through option 4.
                ...
                if int_choice in menu_dispatch:
                    break
                else:
...
class subMenu(object):
    ...
    def create_report():
        '''
        This method takes the table tuple and creates a summary report for the user (name, number of gifts,
        total lifetime amount in gifts, and the average amount given. The code sorts the list rows into
         the largest amount amount give to the least amount given.
        :return:
        '''
        lifetime_donations = int()
        donation_count = int()
        lst_table = []
        # Creates a new list [name, number of gifts, average amount given, total lifetime amount in gifts]
        # to be used to create the summary chart.
        for row in tup_donor_names:
            for value in range(2, len(row)):
                lifetime_donations += row[value]
                donation_count += 1
            lst_table.append([row[0], row[1], lifetime_donations/donation_count, lifetime_donations])
            lifetime_donations = 0
            donation_count = 0
        # Reverses the list
        [row.reverse() for row in lst_table]  # --> Added list comprehension
...
def update_donor(donor_name, donation_amount):
    ...
    lst_donor_name = [tup_donor_names[row][0] for row in range(len(tup_donor_names))]  # --> Added list comprehension
...
def donor_list_creation():
    ...
    lst_donors = [row[0] for row in tup_donor_names]  # --> Added list comprehension
    ...
    for heading in lst_headers:
        print("\n"+("{0:{padding}}".format(heading, padding=max_length)), end=" ")
    # Creating the donor list
    for row in lst_donors:
        print("\n"+("{0:{padding}}".format(row, padding=max_length)), end=" ")
        print()

def send_thankyou_option():
    '''
...
menu_dispatch = {1 : send_thankyou_option,
                 2 : create_report_option,
                 3 : send_letters_option,
