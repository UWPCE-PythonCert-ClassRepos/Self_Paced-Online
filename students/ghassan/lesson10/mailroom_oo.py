class Donor:
    ...
    def mult_donations_by(self, factor, min_donation=None, max_donation=None):
        new_donations = self.donations
        if min_donation:
            new_donations = list(
                filter(lambda x: x > min_donation, new_donations)
                )
        if max_donation:
            new_donations = list(
                filter(lambda x: x < max_donation, new_donations)
                )
        new_donations = list(
            map(lambda x: x * factor, new_donations)
            )
